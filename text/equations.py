...
bh_10d = MathTex("d s_{10}^2","=","-h[r]^{-2} f[r] dt^2+h[r]\\left(f[r]^{-1} d r^2+r^2 d \\Omega_{3}^{2}\\right)","+","L^2 \\sum_{i=1}^{3}\\left\{d \\sigma_i^2+\\sigma_i^2\\left(d \\phi_i+L^{-1} A(r)\\right)^2\\right\}")
bh_10d_rs = MathTex("d s_{10}^2","=","-g[z]dt^2+g[z]^{-1}dz^2+z^2d \\Omega_3^2","+","L^2 \\sum_{i=1}^3\\left\{d \\sigma_i^2+\\sigma_i^2\\left(d \\phi_i+L^{-1} A(z)\\right)^2\\right\}")
g_rs= MathTex("g[z]=1+k^2 z^2-\\frac{2 \\kappa_5 \\mu}{z^2}+\\frac{\\kappa_5^2 \\theta^2}{z^4}")

## Classical Cosmo eqs

friedmann_ind= MathTex("\\left(\\frac{\\dot{a}}{a}\\right)^{2}","=", "-\\frac{1}{a^{2}}","+", "\\frac{8 \\pi}{3}","2  \\left(\\frac{1}{k_{-}}-\\frac{1}{k_{+}}\\right)^{-1} G_{5}"," \\left(\\sigma_{cr}- \\sigma\\right)","+ \\mathcal{O}(\\epsilon^{2})")
...
friedmann_ind_bh_rs= MathTex("\\dot{a}^2=\\frac{1}{4 \\sigma^2 a^2}\\left[\\left(k_{-}^2-k_{+}^2\\right) a^2+\\left(f_{-}(a)-f_{+}(a)\\right)\\right]^2+\\frac{\\sigma^2}{4} a^2-\\frac{1}{2}\\left[\\left(f_{-}(a)+f_{+}(a)\\right)+\\left(k_{-}^2+k_{+}^2\\right) a^2\\right]")
friedmann_ind_bh_rs_junc= MathTex("\\dot{\\mathcal{Z}}^2","=","-f(\\mathcal{Z})-k^2 \\mathcal{Z}^2+\\frac{k^2}{k^2 J_c^2+\\mathcal{Z}^6}\\left(\\mathcal{Z}^4-\\mathcal{Z}_H^4+\\frac{\\theta \\kappa_5 J_c}{\\mathcal{Z}_H^2}\\left(1-\\frac{\\mathcal{Z}_H^2}{\\mathcal{Z}^2}\\right)\\right)^2")
friedmann_ind_bh_dbi= MathTex("\\dot{a}^2","=","-f(a)-k^2 a^2+\\frac{k^2}{a^6}\\left(a^4-a_H^4-\\frac{\\kappa_5^2 \\theta \\Delta \\theta}{a_H^2 k \\Delta k}\\left(1-\\frac{a_H^2}{a^2}\\right)\\right)^2")
...
dbi_action = MathTex("S_{D 3}","=","-T_3 \\int d^4 \\xi \\sqrt{- det P[G]}","+","T_3 \\int P\\left[C_4\\right]")
hamiltonian_from_dbi = MathTex("\mathcal{H}","=","2 \\pi^2 T_3\\left(\\sqrt{-g_{tt}\\left(\\mathcal{Z}^6+\\frac{J_C^2}{L^2}\\right)} \\sqrt{1+g_{zz} \\dot{\\mathcal{Z}}^2}-\\left(C_4\\right)_{t}-\\frac{A_t}{L}", "J_c","\\right)")
...
junction_bh_rs = MathTex("T_3=\\frac{3}{\\kappa_5^2}\\left(\\sqrt{\\frac{g_{-}(a)+\\dot{a}^2}{a^2}}-\\sqrt{\\frac{g_{+}(a)+\\dot{a}^2}{a^2}}\\right)")
